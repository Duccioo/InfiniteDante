...
RL_ITERS = 400  # Increased iterations for better convergence
RL_LR = 1e-5  # Very low LR for stability
RL_BATCH_SIZE = 2  # Single sample per step (faster)
...
TEMPERATURE = 0.7  # Sampling temperature (reduced for stability)
...
class REINFORCETrainer:
    ...
    def __init__(
        self, model, encode_fn, decode_fn, vocab_size, reference_model=None, lr=RL_LR, kl_coeff=KL_COEFF
    ):
        self.model = model
    ...
    def generate_and_compute_reward(
        self, prompt: str, max_tokens: int = GENERATION_LENGTH
        ...
        self.model.eval()
        generated_idx, log_probs = self.model.generate_with_logprobs(idx, max_tokens, temperature=TEMPERATURE)
        self.model.train()
    ...
    def compute_kl_penalty(self, idx, log_probs):
        ...
        with torch.no_grad():
            ...
            # Approximate KL
            kl = (
                log_probs.exp() * (log_probs - ref_log_probs.gather(-1, idx[:, 1:].unsqueeze(-1)).squeeze(-1))
            ).sum()
...
def main():
    ...
    # Training history
    history = {"reward": [], "syllables": [], "rhyme": [], "structure": [], "loss": []}
